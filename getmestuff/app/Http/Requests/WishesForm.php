...
class WishesForm extends FormRequest
{
    ...
    public function save()
    {
        ...
        $wish = $this->intersect($this->wishFields);
        $address = collect($this->intersect($this->addressFields))
            ->map(function ($item) {return ucwords($item);})
            ->toArray();

        $wish['address'] = $address;

        $this->checkItem($wish['item']);
    ...
    protected function checkItem($str) {
        $string = explode(' ', $str);
        if (count($string) > 5) {
            throw new \Exception('Maximum of 5 words for item field.');
        }
